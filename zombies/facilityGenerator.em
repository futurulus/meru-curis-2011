...
function meshFactory(obj, direction, anchor, space, type) {
    return function(presence) {
        presence.loadMesh(function() {
            presence.meshType = type;
            system.__debugPrint("\nCreating a presence of type "+presence.meshType+"\n");
...
function createFacility(type, direction, anchor, space) {
    ...
    //else {
        ...
        for(var i = 0; i < meshes.length; i++) {
            var createMesh = meshFactory(meshes[i], dir, anchor, space, type);
